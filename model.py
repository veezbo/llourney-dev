...
class LLourney(nn.Module):
    ...
    def forward(self, img: Tensor, input_text: list[str], timestep: list[int]) -> Tensor:
        # img (B, 3, I, I)  NOTE: May have to reshape in order to meet this Pytorch convention
        # input_text [B]: str
        # timestep [B]: [0, T)
        B, _, input_img_dim, _ = img.shape
        assert B == len(input_text), "Batch dim in img must match length of input texts"
        assert B == len(timestep), "Batch dim in img must match length of timesteps"
        
        # NOTE: We may want to expose access to this elsewhere and/or return in output
        latent_image = self.vae.encode(img).latent_dist.mean  # (B, LC, L, L)
        ...
        timestep_emb = self.timestep_embed(torch.tensor(timestep, device=DEVICE)).unsqueeze(dim=1)  # (B, 1, C)
        ...
        transformer_emb = self.llama(inputs_embeds=transformer_emb).last_hidden_state  # (B, S, C)
